...
class CoTAttention(nn.Module):
    ...
    def __init__(self, dim=512, kernel_size=3, res=False):
        ...
        self.value_embed = nn.Sequential(
            nn.Conv3d(dim, dim, 1, bias=False), nn.BatchNorm3d(dim)
        )
...
class UNet3D(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, Dmax=128, Hmax=128, Wmax=128):
        super(UNet3D, self).__init__()
        self._coord = False  # 或者根据需要赋予True
...
class SegAirwayModel(nn.Module):
    def __init__(self, in_channels, out_channels, layer_order="gcr", **kwargs):
        ...
        self.unet_3d = UNet3D(in_channels, out_channels)
