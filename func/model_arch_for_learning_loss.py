...
class SegAirwayModel(nn.Module):
    ...
    def model_info(self):
        message = "本模型为model_arch_for_learning_loss.py,使用原本的navi_airway"
        flag = "navi_airway"
        return message, flag
...
class LossNet(nn.Module):
    def __init__(
        self,
        feature_sizes=[32, 16, 8, 4],  # 每个阶段的特征尺寸
        num_channels=[64, 128, 256, 512],  # 每个阶段的通道数
        interm_dim=128,  # 最终输出之前的中间维度
    ):
        super(LossNet, self).__init__()

        # 3D AvgPool 层
        self.GAP1 = nn.AvgPool3d(feature_sizes[0])  # 输入：[8, 32, 32, 128, 128]
        self.GAP2 = nn.AvgPool3d(feature_sizes[1])  # 输入：[8, 64, 16, 64, 64]
        self.GAP3 = nn.AvgPool3d(feature_sizes[2])  # 输入：[8, 128, 8, 32, 32]
        self.GAP4 = nn.AvgPool3d(feature_sizes[3])  # 输入：[8, 256, 4, 16, 16]

        # 根据池化后的尺寸调整 Linear 层的输入大小
        self.FC1 = nn.Linear(512, interm_dim)  # 特征[0]池化后展平尺寸为512
        self.FC2 = nn.Linear(1024, interm_dim)  # 特征[1]池化后展平尺寸为1024
        self.FC3 = nn.Linear(2048, interm_dim)  # 特征[2]池化后展平尺寸为2048
        self.FC4 = nn.Linear(4096, interm_dim)  # 特征[3]池化后展平尺寸为4096
    ...
    def forward(self, features):
        # 每个特征图经过3D池化层
        out1 = self.GAP1(features[0])  # 输出形状：[8, 32, 1, 4, 4]
        out1 = out1.view(out1.size(0), -1)  # 展平为 [8, 512]
        out1 = F.relu(self.FC1(out1))

        out2 = self.GAP2(features[1])  # 输出形状：[8, 64, 1, 4, 4]
        out2 = out2.view(out2.size(0), -1)  # 展平为 [8, 1024]
        out2 = F.relu(self.FC2(out2))

        out3 = self.GAP3(features[2])  # 输出形状：[8, 128, 1, 4, 4]
        out3 = out3.view(out3.size(0), -1)  # 展平为 [8, 2048]
        out3 = F.relu(self.FC3(out3))

        out4 = self.GAP4(features[3])  # 输出形状：[8, 256, 1, 4, 4]
        out4 = out4.view(out4.size(0), -1)  # 展平为 [8, 4096]
        out4 = F.relu(self.FC4(out4))

        # 将四个分支的输出拼接后，传入最终的线性层
