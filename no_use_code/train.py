...
from func.ulti import load_obj,save_obj
...
# settings
# ----------
# save_path = 'checkpoint/checkpoint_sample2_test.pkl'#测试用
save_path = 'checkpoint/exact_17.pkl'
need_resume = True
...
#     "/train_dataset_info_EXACT09_LIDC_IDRI_crops_128_extended_more_high_generation_1"
path_dataset_info_more_focus_on_airways_of_low_generation = \
    "/home/cs22-wangc/now/NaviAirway/abc_train_dataset_info_EXACT09_LIDC_IDRI_crops_128_extended_more_low_gen_10"
path_dataset_info_more_focus_on_airways_of_high_generation = \
    "/home/cs22-wangc/now/NaviAirway/abc_train_dataset_info_EXACT09_LIDC_IDRI_crops_128_extended_more_high_gen_10"


learning_rate = 1e-5
max_epoch = 50#50
freq_switch_of_train_mode_high_low_generation = 1
num_samples_of_each_epoch = 200000#2000
batch_size = 4
...
# print(dataset_info_more_focus_on_airways_of_low_generation)
...
temp1=train_dataset_more_focus_on_airways_of_low_generation
# print(len(temp1),temp1[1].keys(),             type(temp1[1]['label']),   temp1[1]['label'].shape)
#       36290     dict_keys(['image', 'label']) <class 'torch.Tensor'>     torch.Size([1, 32, 32, 32])

# train_file_format = '.npy'
# crop_size = (32, 128, 128)
# windowMin_CT_img_HU = -1000 # min of CT image HU value
# windowMax_CT_img_HU = 600 # max of CT image HU value
#上面的是train.py函数开头提供的参数，用于下面的set_para,
# 有两个地方变化,一是windowMax预设(指set_para的预设)为150，train的是600，crop_size预设为32，train的是(32, 128, 128)
#不同的参数以train的为准，在定义dataset时，数据还没有被取出来，也就没有使用getitem,只有在使用dataloader时，数据才被取了出来
...
for ith_epoch in range(0, max_epoch):#注意ith_epoch从0开始
    ...
    # print(np.floor(ith_epoch/freq_switch_of_train_mode_high_low_generation))
    #不太懂为什么进行这个if判断
    #np.floor是numpy库中的一个函数，用于返回小于或等于输入值的最大整数，其返回值类型与输入值相同。
    if np.floor(ith_epoch/freq_switch_of_train_mode_high_low_generation)%2==0 or \
        ...
        #对于这里low和high，仅数据集是不一样的，其他都一样


        #freq_switch_of_train_mode_high_low_generation = 1
        print("train_more_focus_on_airways_of_low_generation")
        #num_samples_of_each_epoch=4
        #dataset_info_more_focus_on_airways_of_low_generation是字典，除了包括image和label的地址，同时还包含了一些参数，详见上文
        # print(len(dataset_info_more_focus_on_airways_of_low_generation),len(train_dataset_more_focus_on_airways_of_low_generation))
        sampler_of_airways_of_low_generation = RandomSampler(train_dataset_more_focus_on_airways_of_low_generation,
                                                             num_samples = min(num_samples_of_each_epoch, 
                                                                               len(dataset_info_more_focus_on_airways_of_low_generation)), 
                                                             replacement = True)
        # 具体来说，这个采样器叫做随机采样器（RandomSampler），
        # 它会从数据集中随机选择一些样本。其中，num_samples表示每个epoch中需要采样的样本数量，
        # 而len(train_dataset)表示数据集的总样本数。
        # min(num_samples_of_each_epoch, len(train_dataset))这个语句表示需要采样的样本数量，
        # 是每个epoch中需要采样的数量和数据集总样本数中的较小值。
        # replacement=True表示采样时可以重复选择某些样本，如果replacement=False，则不会重复选择样本。
        ...
        # 这段代码是用来创建一个数据加载器（dataset_loader），它是用来批量读取数据的工具，
        # 可以将训练集中的数据按照batch_size大小分成多个批次（batch），每个批次包含多个样本。

        # 具体来说，
        # 这个数据加载器会加载训练集（rain_dataset_more_focus_on_airways_of_low_generation）中的数据，
        # batch_size表示每个批次中的样本数量，
        # sampler参数指定了前面的采样器(采样器决定了在数据集里如何选择样本)。
        # num_workers参数表示使用多少个进程来并行读取数据，
        # pin_memory=True表示将数据存储在CUDA固定内存中以加快数据传输速度，
        # persistent_workers=(num_workers > 1)表示保留数据加载器的工作进程以加快下一次数据加载的速度，当num_workers大于1时才会生效
    else:
    ...
    # #总之，这个if/else的结果是:
    #     1.选择train_more_focus_on_airways_of  low_generation/high_generation
    #     2.生成采样器sampler_of_airways_of_xxx
    #     3.生成dataset_loader#dataloader只有数据集不同、采样方式不同，但没仔细看哪里不同

    len_dataset_loader = len(dataset_loader)#有多少个batch 比如有100个样本，batch_size=5，len(dataset_loader)=20
